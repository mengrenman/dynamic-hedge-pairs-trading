"""Build notebooks/pairs_trading_07_daily_cointegration.ipynb (cells only; outputs from execute.py).
...
## `pairs_trading_07_daily_cointegration.ipynb`

Notebook 06 built a point-in-time universe from the day lake and fixed the price basis. This notebook
...
Benjamini–Hochberg discoveries era by era, and the identity of what actually passes. Notebook 08 then
trades the result.
...
TEST_SYMBOLS = sorted(t for t in bars.index.get_level_values("ticker").unique()
                      if re.fullmatch(r"Z[A-Z]ZZT", str(t)))     # NASDAQ test symbols; see notebook 06
...
The universe rules are notebook 06's, including the three that a curated index list would have hidden:
...
nothing, the two selections should perform alike in notebook 08.
""")
code(r"""
...
Notebook 08 trades all three over the same twenty years with the same hedge, costs and universe.
""")
code(r"""
rules = {}
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=Path, default=Path(__file__).resolve().parent.parent / "pairs_trading_07_daily_cointegration.ipynb")
    args = ap.parse_args()
