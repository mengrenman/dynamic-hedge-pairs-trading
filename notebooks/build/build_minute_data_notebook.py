"""Build notebooks/pairs_trading_09_minute_data.ipynb (cells only; outputs are produced by execute.py).
...
## `pairs_trading_09_minute_data.ipynb`
...
* bars are labelled by their **start** minute (the lake's convention): the close of the bar labelled 09:30
  is the last trade before 09:31. Signal timing in notebook 10 is built on this.
""")
code(r"""
...
The full minute grid is loaded only for the tickers in the candidate pairs (both spans, since notebook 11
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=Path, default=Path(__file__).resolve().parent.parent / "pairs_trading_09_minute_data.ipynb")
    args = ap.parse_args()
