"""Build notebooks/pairs_trading_10_minute_data.ipynb (cells only; outputs are produced by execute.py).
...
## `pairs_trading_10_minute_data.ipynb`
...
is 2022-01-03 → 2024-12-31; 2025 is reserved as the hold-out for notebook 12. The cointegration screen
below, and every design choice in notebook 11, use the training span only. One caveat worth stating: the
...
N_CANDIDATES = 24                                     # pairs carried into notebooks 11 and 12
...
* bars are labelled by their **start** minute (the lake's convention): the close of the bar labelled 09:30
  is the last trade before 09:31. Signal timing in notebook 11 is built on this.
""")
code(r"""
...
The candidates handed to notebooks 11 and 12 are the passing pairs with the smallest BH-corrected
...
The full minute grid is loaded only for the tickers in the candidate pairs (both spans, since notebook 12
...
a filter that re-estimates the hedge every minute would absorb part of what we want to measure (notebook 11
...
* **Fast hedges are dangerous.** A Kalman filter re-estimating the hedge every minute will absorb the
  slow reversion into its state and leave a residual that is white at the bar scale (notebook 11 shows this
  directly). The hedge should move at most daily.

Notebook 11 turns these into a walk-forward comparison: hedge cadence, sampling frequency, session rule,
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=Path, default=Path(__file__).resolve().parent.parent / "pairs_trading_10_minute_data.ipynb")
    args = ap.parse_args()
