...
* `freq="5min"` (any whole number of minutes) aggregates the grid per session — bars labeled by their
...
* bars are labeled by their **start** minute (the lake's convention): the close of the bar labeled 09:30
...
def rel(df):  # normalize each column by its 30-minute value
