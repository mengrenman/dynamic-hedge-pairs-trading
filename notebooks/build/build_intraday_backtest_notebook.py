"""Build notebooks/pairs_trading_11_intraday_backtest.ipynb (cells only; outputs are produced by execute.py).
...
## `pairs_trading_11_intraday_backtest.ipynb`

Notebook 10 established the candidates and the facts that matter: the cointegrated spreads of liquid
...
The 2025 hold-out is untouched; notebook 12 spends it once. The cold run takes about 10 minutes on 16
...
Everything comes from the caches notebook 10 writes; if they are missing they are rebuilt here with the
...
commission and slippage of 1 bp per leg per side plus half the Roll spread of each leg (notebook 10), a
...
""")
md(r"""
The ordering is the one notebook 10 predicted, and the mechanism is visible in the last two columns.
...
pairs instead of eight. Notebook 10 measured 40% of the spread's variance overnight; an intraday-only rule
...
threshold (Sharpe −2.3 to −2.8) — the economics of notebook 10 §6.4 in action, a short window scores
...
## 4. The design carried into notebook 12
...
grid's best cell (look-back 20 sessions, entry 2.5; 2.5) is recorded alongside for notebook 12.
""")
code(r"""
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=Path, default=Path(__file__).resolve().parent.parent / "pairs_trading_11_intraday_backtest.ipynb")
    args = ap.parse_args()
