"""Build notebooks/pairs_trading_17_intraday_portfolio.ipynb (cells only; outputs are produced by execute.py).
...
## `pairs_trading_17_intraday_portfolio.ipynb`

Notebook 16 chose an intraday design on pooled out-of-fold Sharpe over 2022–2024. This notebook spends the
...
# the design chosen in notebook 16 (falls back to its documented result if the file is missing)
...
print("pairs", pairs.__version__, "| design from notebook 16:", HEDGE, "@", K, "min;", CFG, "| grid best:", CFG_GRID)
""")
...
Same caches and the same fold engine as notebook 16 (repeated here so the notebook stands alone), plus the
...
* **intraday** — the design notebook 16 carried forward: its best hedge and sampling frequency with the
...
* **intraday, grid best** — the same hedge and frequency with the look-back/entry cell that maximised the
  training-span pooled Sharpe in notebook 16 (the "tuned" version);
...
lag so nothing leaks overnight. Shown on the training-span out-of-fold states from notebook 16 (25 folds,
...
price. The corollary matters for the one-minute variant of notebook 16, whose 608 trades over the
...
went wrong in 2025 is a missing gross edge, not microstructure. Costs are part of why one-minute bars trail in notebook 16 — they pay
...
share of the spread's variance notebook 15 measured — and that is with positions entered and exited
...
(notebook 16 §3.1); look-backs of a session or less lose at every threshold; flattening at the close
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", type=Path, default=Path(__file__).resolve().parent.parent / "pairs_trading_17_intraday_portfolio.ipynb")
    args = ap.parse_args()
