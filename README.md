...
executes any notebook in place. Notebooks 07–09 and 13 read a local day-bar lake, and notebooks 10–12 a
...
(Yahoo Finance). Notebooks 07–09 use the local Polygon-derived **day lake** and 09–11 the **minute
lake**, neither of which touches the network.
...
│  ├─ pairs_trading_10_minute_data.ipynb            # minute lake, session grid, liquidity, intraday microstructure of the spread
│  ├─ pairs_trading_11_intraday_backtest.ipynb      # intraday walk-forward: hedge cadence, sampling frequency, session rule, thresholds
│  ├─ pairs_trading_12_intraday_portfolio.ipynb     # 2025 hold-out once, latency, costs, capacity, P&L attribution, portfolio
│  ├─ pairs_trading_13_daily_cross_sectional.ipynb  # PCA eigenportfolios, residual reversal on the whole cross-section, cost sweep
│  └─ build/                               # scripts that generate nb04–nb13, the tv and the visualisation notebooks + execute.py runner
│
...
- **Execution lag:** `generate_pair_signals` uses next-bar execution (backtest-safe by construction). On minute bars that fill is optimistic — the decision bar's own close — so `pairs_trading_12_intraday_portfolio.ipynb` also reports `exec_lag=2` and `3`.
