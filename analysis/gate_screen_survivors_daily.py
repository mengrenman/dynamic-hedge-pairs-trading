...
actually warrant a *dynamic* hedge? Writes cache/tv_gate_survivors_daily.parquet.

This is the DAILY-LEVELS companion to the weekly run: the same 2-year formation windows, but the
series notebook 10 actually screened and notebook 11 actually fits its hedge to, rather than the
weekly logs notebook 07 used. ~115s a pair at B=199, so ~5.7 h on 12 cores; resumable.

The traded pair-folds are dispatched FIRST, so the answer that bears on the strategy lands in the
first hour rather than the sixth.
...
OUT = Path("cache/tv_gate_survivors_daily.parquet")
...
for r in surv.itertuples():
    ...
    jobs.append(({**meta, "n_daily": len(d)}, d["y"].to_numpy(), d["x"].to_numpy()))
del PX
_rule = pd.read_parquet("cache/day_rule_bh_dual.parquet")
_rule["formation"] = pd.to_datetime(_rule["formation"])
_traded = set(zip(*[_rule.sort_values("eg_p_fdr").groupby("formation").head(20)[c]
                    for c in ("formation", "ticker1", "ticker2")]))
for m, _y, _x in jobs:
    m["traded"] = (m["formation"], m["t1"], m["t2"]) in _traded
jobs.sort(key=lambda j: not j[0]["traded"])        # traded pair-folds first
print(f"{len(surv)} survivors: {len(jobs)} to run ({sum(j[0]['traded'] for j in jobs)} of them "
      f"traded by notebook 11, dispatched first), {len(skipped)} unusable "
